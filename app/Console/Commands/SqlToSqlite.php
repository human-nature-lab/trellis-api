...
class SqlToSqlite extends Command {
  ...
  private function copyQuery (string $table, $q) {
    $c = 0;
    $q->chunkById($this->option('chunk-size'), function ($rows) use ($table, &$c) {
      $insertRows = [];
      foreach ($rows as $i => $row) {
        $insertRows[$i] = (array)$row;
      }
